...
- **FR-018**: System MUST ship with one pre-populated roadmap, "AI/MLOps & Agentic AI Infrastructure," containing six steps in a fixed order, each with a title, description, a concrete checklist (10+ actionable tasks), curated `course_links` to verified public resources, and a `documents` entry linking to an original, in-depth reference guide for that step (rendered at `/roadmap/docs?doc=<slug>`, source in `static/roadmap-docs/<slug>.md`) — see `scripts/seed_roadmap.py` for the authored content. `notes` begins empty for the user to fill in; checklist items begin unchecked (`done: false`), so overall completion is still 0% immediately after seeding.
