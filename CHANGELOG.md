...
#### Notes Workspace: new tool combining Notepad++, OneNote, and Obsidian (`static/notes.html`, `static/notes.js`, `static/notes-links.js`, `static/notes.css`)
- New 12th DevSuite tool at `/notes`: a multi-tab Markdown editor (Monaco, reusing the already-vendored editor and its Markdown language chunk) organized as Notebook → Section → Page (drag-to-reorder sections and pages), with Obsidian-style `[[wiki-links]]` (autocomplete, create-on-link, automatic backlinks panel, rename propagation), inline `#tags` with a tag browser, and workspace-wide full-text search — see `specs/017-notes-workspace/`.
- **Encrypted, master-password gated** — same client-side model as Secret Vault: WebCrypto PBKDF2-SHA256/310k → AES-256-GCM, with its own independent salt stored alongside the ciphertext in a new `notes` DevDB store. The backend only ever sees an opaque blob via `GET/POST /api/notes` (byte-for-byte mirrors `/api/vault`'s contract) and never decrypts it. Reuses `auth-guard.js` for the unlock screen/session rather than duplicating Vault's own lock overlay.
- **New dependency: DOMPurify** (`static/libs/dompurify.min.js`, 3.4.13, self-hosted) — sanitizes `marked`-rendered Markdown before it's ever inserted into the DOM for the live preview pane. `marked` (already vendored) has no built-in sanitizer, and rendering user-authored Markdown to HTML on every page reopen without sanitizing it is a stored-XSS vector; DOMPurify is the standard pairing `marked`'s own docs recommend. Added to `specs/SPEC.md §11.3` and `UPGRADE_PLAN.md`. Loads before `require.min.js` per the existing UMD-before-RequireJS rule (`tests/python/test_asset_order.py` extended to cover it).
- All wiki-link resolution, backlinks, tag indexing, and full-text search run client-side over the already-decrypted tree (`static/notes-links.js`, a pure Node/browser dual-export module) — the server has no search or link-resolution surface, consistent with never holding plaintext.
