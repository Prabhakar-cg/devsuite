...
> Source material: migrated from `CLAUDE.md` and `specs/SPEC.md` §2/§7/§10/§12 on 2026-06-10.
> `specs/SPEC.md` remains the detailed master specification; this constitution holds the
...
`specs/SPEC.md` is the single source of truth. Any change to behavior, API shape, UI, or
...
security rule must appear in `specs/SPEC.md`, added in the same commit that introduces it.
...
the `README.md` badge, the `CHANGELOG.md` heading, and `specs/SPEC.md` §1.3. Changelog
...
- Spec-kit flow for new work: `/speckit-specify` → (optional `/speckit-clarify`) →
  ...
  `specs/SPEC.md` so it stays the single source of truth.
...
`specs/SPEC.md`, and `CLAUDE.md` together, with the change recorded in `CHANGELOG.md`.
